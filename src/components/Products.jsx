...
const Products = () => (
  <section id="products" className="py-12 px-6 bg-white text-center">
    ...
    <div className="grid md:grid-cols-3 gap-8">
      {products.map((p) => (
        <div key={p.id} className="w-full h-100 bg-amber-300 border rounded-lg p-4 shadow hover:scale-105 transition">
          <img src={p.img} alt={p.name} className="w-full h-full object-cover rounded-lg" />
          <h4 className="mt-4 font-semibold text-lg">{p.name}</h4>
          <p className="text-yellow-700 mt-1">{p.price}</p>
        </div>
      ))}
    </div>
  </section>
